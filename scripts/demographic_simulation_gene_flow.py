...
# We allow gene flow between taxa A and B between 1 and 0.5 Ma
...
<<<<<<< HEAD:migration0.2/scripts/demographic_simulation.py
...
=======
>>>>>>> e63f2123e2ec9a81adeec34456d1cf306983b0f5:scripts/demographic_simulation_gene_flow.py

# Simulating gene flow
demography.add_migration_rate_change(time=1000000, rate=0.2, source="A", dest="B")
demography.add_migration_rate_change(time=1000000, rate=0.2, source="B", dest="A")
...
ts = msprime.sim_ancestry(samples={"A": 10, "B": 10, "C": 10, "D": 10}, 
...
try:
    ...
    pop_mrcas["ABCD"] = tree.mrca(pop_mrcas["ABC"], pop_mrcas["D"])
...
pop_newick = tree.as_newick(root=pop_mrcas["ABCD"])
print("Population-Level Newick Tree:", pop_newick)

# Simplify the tree sequence to retain only species-level structure
simplified_ts = ts.simplify(samples=[pop_mrcas["A"], pop_mrcas["B"], pop_mrcas["C"], pop_mrcas["D"]], keep_input_roots=True)
